...
def riwayatConsumable(consumable_history, consumable, user, value_pos):
    ret = []
    for i in range(len(consumable_history) - value_pos,
                   len(consumable_history) - 5 - value_pos if len(consumable_history) >= 5 + value_pos else 0, -1):
...
def riwayatGadget(gadget_return_history, gadget_borrow_history, gadget, user, value_pos):
    ...
    for i in range(len(gadget_return_history) - value_pos,
                   len(gadget_return_history) - 5 - value_pos if len(gadget_return_history) >= 5 + value_pos else 0,
                   -1):
